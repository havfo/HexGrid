...
vec4 getTerrainColor (int index) {
    vec3 sample = vec3(worldPosition.xz * 0.1, terrain[index]);
...
void main(){
    ...
    // ***********************
    ...
    #if defined(GRID_LINES) && defined(SHOW_GRID)
      vec2 gridUV = worldPosition.xz;
      gridUV.x *= (1.0 / (4.0 * 8.66025404));
      gridUV.y *= (1.0 / (2.0 * 15.0));
      gridUV.x += 0.02;
      gridUV.y += 0.02;
      vec4 grid = texture2D(m_GridLines, gridUV);
    #else
      vec4 grid = vec4(1.0);
    #endif
      ...
      vec4 specularColor = vec4(1.0);
       ...
       gl_FragColor.rgb =  AmbientSum       * diffuseColor.rgb * grid.rgb +
                           DiffuseSum.rgb   * diffuseColor.rgb  * vec3(light.x) +
